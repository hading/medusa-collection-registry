class FileFormatProfilesController < ApplicationController
  ...
  def destroy
    ...
    if @file_format_profile.destroy
      ...
      redirect_back alert: 'Unable to destroy this file rendering profile', fallback_location: file_format_profiles_path
    end
  end
  ...
  def allowed_params
    params[:file_format_profile].permit(:name, :status, :software, :software_version, :os_environment, :os_version,
                                        :notes, :logical_extensions_string, file_format_ids: [], content_type_ids: [])
  end
